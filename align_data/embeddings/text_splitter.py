...
def boundary_cost(boundary: Boundary, preferred_pos: int, doc_tokens: TokenMapper) -> Tuple[int, ...]:
...
def chunks(doc: str, min_chunk_length: int = 700, max_chunk_length: int = 1500, 
          preferred_length_tokens: int = 1000, chunk_max_overlap: int=100) -> Tuple[List[Chunk], Dict[str, int]]:
    """
    ...
    assert max_chunk_length >= 2 * min_chunk_length, f"error: max_chunk_length ({max_chunk_length}) should be at least 2x min_chunk_length ({min_chunk_length}) so chunk merging is more likely to be valid"
    ...
    while start_pos < len(doc):
        ...
        if len(result_chunks) > 0 and chunk_max_overlap > 0:
            overlap_start = doc_tokens.tokens_forward_from_char(start_pos, -chunk_max_overlap)
            ...
            if overlap_boundaries:
                ...
                best_overlap_boundary = min(overlap_boundaries, 
        ...
        preferred_pos = doc_tokens.tokens_forward_from_char(actual_start_pos, preferred_length_tokens)
        if preferred_pos >= len(doc):
            result_chunks.append(Chunk(actual_start_pos, doc[actual_start_pos:], 0, remaining_tokens, BPat('last_chunk_by_chars', [r"$(?!.)"], None, None)))
        ...
        for boundary in boundaries:
            chunk_tokens = doc_tokens.char_span_to_tokens(actual_start_pos, boundary.pos)
            if chunk_tokens > max_chunk_length: break
        ...
        if not valid_boundaries:
            print("wtf, no valid boundaries or short boundaries, wtf? should only happen if someone gives us a string with no spaces. returning preferred chunk without regard to word cutting")
            result_chunks.append(Chunk(actual_start_pos, doc[actual_start_pos:preferred_pos], len(doc)-preferred_pos, doc_tokens.char_span_to_tokens(actual_start_pos, preferred_pos),  BPat('chars', [r"."], None, None)))
            boundary_stats['emergency_fallback'] += 1
            start_pos = preferred_pos
            continue
        
        # Score and select best boundary
        best_boundary = min(valid_boundaries, 
                          key=lambda b: boundary_cost(b, preferred_pos, doc_tokens))
        
        result_chunks.append(Chunk(actual_start_pos, doc[actual_start_pos:best_boundary.pos], len(doc) - best_boundary.pos, doc_tokens.char_span_to_tokens(actual_start_pos, best_boundary.pos), best_boundary.pattern))
    ...
    if len(result_chunks) >= 2 and doc_tokens.char_span_to_tokens(result_chunks[-1].start_pos, result_chunks[-1].start_pos + len(result_chunks[-1].value)) < min_chunk_length:
        result_chunks[-2:] = [Chunk(result_chunks[-2].start_pos, doc[result_chunks[-2].start_pos:], 0, doc_tokens.char_span_to_tokens(result_chunks[-2].start_pos, len(doc)), BPat('merge_last', [result_chunks[-2].pat, result_chunks[-1].pat], None, None))]
...
def split_text(text: str) -> list[str]:
    ...
    chunks_list, _ = chunks(
        ...
        preferred_length_tokens=preferred_length_tokens,
