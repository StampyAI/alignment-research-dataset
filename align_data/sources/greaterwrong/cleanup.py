...
def cleanup_posts_by_tags(source: str, dry_run: bool = False):
    """
    ...
    with make_session() as session:
        ...
        logger.info(
            f"Found {len(posts_to_mark_invalid)} {source} posts that don't meet tag requirements"
        )
        ...
        if not dry_run and posts_to_mark_invalid:
            for post in posts_to_mark_invalid:
                ...
                post.append_comment(
                    "Automatically marked as invalid because it doesn't meet tag requirements"
                )
...
if __name__ == "__main__":
    ...
    if source not in SOURCE_CONFIG:
        logger.error(
            f"No configuration defined for source '{source}'. Available sources: {list(SOURCE_CONFIG.keys())}"
        )
        sys.exit(1)
