...
from align_data.settings import PINECONE_VALUES_DIMS, DEVICE, OPENAI_FINETUNED_LAYER_PATH, OPENAI_CURRENT_BEST_FINETUNED_LAYER_PATH
...
class NonLinearFineTuneModel(nn.Module):
    def __init__(
        self, 
        embedding_dim=PINECONE_VALUES_DIMS, 
        hidden_dim=2000, 
        dropout=0.5
    ):
...
def load_best_model_if_exists(model):
    ...
    if os.path.exists(OPENAI_CURRENT_BEST_FINETUNED_LAYER_PATH):
        model.load_state_dict(torch.load(OPENAI_CURRENT_BEST_FINETUNED_LAYER_PATH, map_location=DEVICE))
        print(f"Loaded model from {OPENAI_CURRENT_BEST_FINETUNED_LAYER_PATH}.")
    else:
        print(f"No saved model found at {OPENAI_CURRENT_BEST_FINETUNED_LAYER_PATH}. Starting from scratch.")
